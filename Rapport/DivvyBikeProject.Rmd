...
La fonction de ce package est de regrouper toutes les manipulations que nous avons apporté sur les données : il permet donc de passer des données brutes (directement téléchargeable sur internet, voir **II**) aux différents jeux de données utilisés dans nos modèles. Nous présenterons les fonctions présentes dans notre package au fur et à mesure des deux premières parties.
...
Nous pouvons voir que les stations vélos ont dans un premier temps été réparti dans le centre puis elles vont ensuite s'étendre vers les banlieues.

Nous nous sommes également rendu compte que certaines stations se déplaçaient de quelques mètres d'une année sur l'autre ou changer de noms. Ces deux observations justifient en partie l'agrégation spatiale fait par la suite.

Regardons maintenant la fréquentation des stations en 2016 : 

![](Frequ.png)

Nous remarquons sur ce graphique, que les vélos les plus utilisés sont surtout ceux du centre ville. Aux vues des profils de nos trajets, il semble que le centre regroupe écoles et lieux de travail (voir profil d'une journée plus bas). 
...
Nous présenterons dans cette partie les manipulations que nous avons effectué sur les données. Cette partie représente une bonne partie de notre travail, elle est donc centrale. Les trois paragraphes suivants permettent de se rendre compte d'une partie des problèmes "simples" que ous avons rencontré et qui a motivé une transformation des données. 

Les données que l'on trouve sur le site de Divvy ne sont pas divisées de la même façon selon les années. Par exemple, pour 2014, nous avons un fichier contenant toutes les données de janvier à juin (Q1Q2) mais pour 2016, nous avons un fichier avec Q1 (janvier à mars) puis un fichier pour avril, un autre pour juin et un dernier pour juillet. Dans notre package, nous prenons en compte les spécificités de chaque année pour homogénéiser.

A l'intérieur de chaque fichier, on trouve des différences au niveau des noms des variables (exemple : '-' une année et '_' une autre).

Un autre problème que l'on a rencontré se trouve au niveau des stations. Nous avons déjà remarqué plus haut que certaines changent de nom et d'autre de place au cours du temps.

<p>&nbsp; </p>

Nous avons réalisé plusieurs agrégations : une agrégation temporelle et une agrégation spatiale. Ces agrégations ont été motivé d'une part par les problèmes précédemment décrit et d'autre part par une volonté de réduire la dimension de notre jeu de données (13.5 millions de trajets et une dizaine de variables).

### Agrégation temporelle

Nous avons compté, pour chaque station, le nombre de vélos entrant (**nbE**) et le nombre de vélos sortant (**nbS**) à chaque heure. L'agrégation au pas horaire nous a paru pertinente car elle permet de garder une maille temporelle assez fine tout en réduisant la dimensionalité de manière efficace. 
...
Pour gérer les données géographiques, nous avons utilisé les packages R *rgdal* et *raster*. Étant donné un découpage spatial de la ville de Chicago contenue dans le Large SpatialPolygonDataFrame *area*, ainsi qu'un fichier *data* regroupant les données agrégées temporellement sur les *stations*, nous obtenons une agrégation spatiale via la fonction **groupByGeospatialData(data, area, stations, year,  save_data = FALSE)**
...
Nous avons proposé $3$ série d'intérêt différentes :
...
Nous estimons que cette dernière statistique est la plus utile, car elle permet de déterminer comment les vélos doivent être déplacer entre les différentes régions afin de s'assurer que les stations ne soient jamais soit complètement vides, soit complètement pleines. 
...
Pour commencer, intéressons_nous à la tendance de nos séries. La série **diff** n'ayant pas de tendance par construction (centrée en zéro), nous allons nous concentrer sur les séries **nbEstat** et **nbSstat**. 
...
Les deux séries se ressemblant beaucoup, nous allons présenté uniquement les résultats pour la variable **nbEstat**.
...
Les visualisations graphiques semblent nous indiquer une double saisonnalité : annuelle et journalière. Pour rechercher la saisonnalité nous avons utilisé la fonction *lm* comme précédemment pour la tendance mais également les modèles additifs généralisés (*gam*). Ces deux méthodes ont aboutit au même résultat.
...
En utilisant les mêmes hyper-paramètre que precedemment, on peut essayer de n'utiliser comme données d'entrainements que les données concernant un district donné pour prédire sur ce district. On appelle ce modèle modèle local, en opposition au modèle précédent, global. Le plus souvent, les modèles locaux sont plus efficaces pour prédire que le modèle global, mais ce n'est pas toujours le cas. Le tableau et le graphique suivant montrent différents comportements typiques (sur le graphique, les prédicitions sont en noir).  
...
Il semble donc que la plupart des variables soient significatives, mais pas la pluviométrie. La encore, on peut tester la différence entre modèles locaux et globaux. 
...
Les modèles locaux sont encore plus efficaces que le modèles global.

# V - Conclusion

blablabla

Nous remerçions Monsieur Goude pour nous avoir aider à réaliser ce projet. 

...
Nous utiliserons comme critère de perfomance le RMSE (écart-type moyen). 
...
Pour chaque jour, on peut calculer une saisonnalité par série de Fourier. Notons qu'on ne considère à chaque fois que les jours du même types, afin d'avoir une différence raisonnable . Ici, un zoom permet de constater que les formes des courbes sont relativement semblable.  Ici, en janvier, il semble que la prédiction (en rouge) soit un peu en avance. Cela est peut être du à des phénomènes saisonniers ou à des changements d'heure. 

![](Saison2017.png).

On obtient les résidus suivants pour 2017: 

![](Residus.png)

Ces résidus sont assez faible, mais on observe toujours une persistance de la saisonnalité

#### Écart

Après avoir enlevé la saisonnalité et la tendance, on essaie d'expliquer ces résidus à l'aide d'un modèle additif en fonction de la température et de la pluviométrie.  On peut un modèle additif pour l'ecart avec un modèle multiplicatif. On obtient pour la prévision sur 2017 un RMSE de 0.50 par station pour le premier, et de 0.48 pour le second, qui semble donc meilleur.

Notons que ce RMSE n'est pas exactement comparable aux modèles suivants , puisque qu'il ne tient pas compte du regroupement par districts. Par manque de temsps, nous n'avons pas pu tous les modèles avec la même fonction objectif pour comparer, mais nous y travaillons.
